...
安装 `rk-memory` 命令行工具并跑通演示闭环（PowerShell 和 Bash 通用）：
...
`python scripts/seed_demo_db.py`、`python researchkb/rk_health.py`、
...
在 Cursor（`~/.cursor/mcp.json`）或 Claude Code（`.mcp.json`）中注册。已安装包
（`python -m pip install -e .`）时：
...
未安装时把 `command` 换成 `python`，
`args` 用 `["<RepoRoot>/researchkb/rk_mcp_server.py", "--root", "<ResearchKBRoot>"]`。
...
- `src/researchkb_agent_memory/`: `rk-memory` CLI 背后的可安装包；下面所有工具都转调它。
